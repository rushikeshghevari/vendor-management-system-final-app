 ...
 *   post:
 *     tags: [Purchase Orders]
 *     summary: Generate a Purchase Order from an Approved Quotation (Department User / HOD / Super Admin)
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             type: object
 *             required: [quotationId, items]
 *             properties:
 *               quotationId: { type: string, description: Approved Quotation id }
 *               items:
 *                 type: array
 *                 minItems: 1
 *                 items:
