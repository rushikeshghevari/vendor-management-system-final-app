...
/**
 * Send a push notification to an entire FCM topic.
 * Used for role-based or department-based broadcast.
 */
export async function sendToTopic(topic: string, payload: {
  title: string;
  body: string;
  ...
  try {
    await messaging.send({
      ...
      android: {
        ...
        notification: {
          channelId: payload.channelId ?? 'vms_default',
          sound:     'default',
    ...
    console.error(`[topic] sendToTopic(${topic}) failed:`, err);
    return false;
  }
}
